  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-(--space-6)">
        <h1 className="text-(--text-3xl) font-bold" style={{ fontFamily: 'var(--font-display)' }}>Products</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-(--color-surface-hover) border-b border-(--color-border)">
                <th className="p-(--space-4) text-(--text-sm) font-semibold text-(--color-text-secondary)">Product</th>
                <th className="p-(--space-4) text-(--text-sm) font-semibold text-(--color-text-secondary)">Status</th>
                <th className="p-(--space-4) text-(--text-sm) font-semibold text-(--color-text-secondary)">Inventory</th>
                <th className="p-(--space-4) text-(--text-sm) font-semibold text-(--color-text-secondary)">Price</th>
            ...
            <tbody className="divide-y divide-(--color-border)">
              {products.map((product) => {
                ...
                return (
                  <tr key={product.id} className="hover:bg-(--color-surface-hover) transition-colors">
                    <td className="p-(--space-4)">
                      <div className="flex items-center gap-(--space-3)">
                        <div className="w-12 h-12 rounded bg-(--color-bg-alt) flex items-center justify-center shrink-0">
                          {product.images[0]?.url ? (
                            <Image src={product.images[0].url} alt={product.name} width={32} height={44} className="h-10 w-auto object-contain" />
                          ) : (
                            <span className="text-(--text-xs) text-(--color-text-muted)">No img</span>
                          )}
                        </div>
                        <div>
                          <p className="font-semibold text-(--text-sm)">{product.name}</p>
                          <p className="text-(--text-xs) text-(--color-text-muted)">SKU: {product.sku}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-(--space-4)">
                    ...
                    <td className="p-(--space-4)">
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${lowStock ? 'bg-(--color-danger)' : 'bg-(--color-success)'}`} />
                        <span className="text-(--text-sm)">{stock} in stock</span>
                      </div>
                    </td>
