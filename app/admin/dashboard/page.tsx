...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      <h1 className="text-(--text-3xl) font-bold mb-(--space-6)" style={{ fontFamily: 'var(--font-display)' }}>Dashboard</h1>
      ...
      {/* Recent Orders */}
      <div className="card">
        <div className="p-(--space-5) border-b border-(--color-border) flex items-center justify-between">
          <h2 className="text-(--text-lg) font-semibold">Recent Orders</h2>
          <Link href="/admin/orders" className="text-(--text-sm) text-(--color-primary) hover:underline">View All</Link>
        </div>
        
        {recentOrders.length === 0 ? (
          <div className="p-(--space-8) text-center text-(--color-text-muted)">
            <p className="text-4xl mb-(--space-3)">📦</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-(--text-sm)">
              <thead className="bg-(--color-surface) text-(--color-text-secondary)">
                <tr>
                  <th className="px-(--space-4) py-(--space-3) font-medium">Order Number</th>
                  <th className="px-(--space-4) py-(--space-3) font-medium">Customer</th>
                  <th className="px-(--space-4) py-(--space-3) font-medium">Date</th>
                  <th className="px-(--space-4) py-(--space-3) font-medium">Status</th>
                  <th className="px-(--space-4) py-(--space-3) font-medium text-right">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-(--color-border)">
                {recentOrders.map((order: any) => (
                  <tr key={order.id} className="hover:bg-(--color-surface-hover) transition-colors">
                    ...
                    <td className="px-(--space-4) py-(--space-3)">{order.user?.name || order.guestEmail || 'Guest'}</td>
                    <td className="px-(--space-4) py-(--space-3) text-(--color-text-muted)">{new Date(order.createdAt).toLocaleDateString()}</td>
                    <td className="px-(--space-4) py-(--space-3)">
                      <span className={`badge ${order.status === 'PENDING' ? 'badge-outline' : order.status === 'DELIVERED' ? 'badge-success' : 'badge-primary'}`}>
                        {order.status}
                      </span>
                    </td>
                    <td className="px-(--space-4) py-(--space-3) text-right font-medium tabular-nums">{formatPrice(Number(order.total))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
