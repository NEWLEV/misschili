...
export default function ReturnsPage() {
  return (
    <div className="section-container section-padding max-w-3xl">
      <h1 className="text-(--text-4xl) font-bold mb-(--space-6)" style={{ fontFamily: 'var(--font-display)' }}>Returns & Refunds</h1>
      <div className="space-y-(--space-5) text-(--color-text-secondary)">
        <h2 className="text-(--text-xl) font-semibold text-(--color-text)">Return Policy</h2>
        <p>We accept returns within 30 days of delivery for <strong>unopened products</strong> in their original packaging.</p>
        <h2 className="text-(--text-xl) font-semibold text-(--color-text) mt-(--space-6)">How to Return</h2>
        <ol className="list-decimal pl-(--space-6) space-y-(--space-2)">
          <li>Email us at <a href="mailto:misschilihotsauce@gmail.com" className="text-(--color-primary)">misschilihotsauce@gmail.com</a> with your order number</li>
        ...
        <h2 className="text-(--text-xl) font-semibold text-(--color-text) mt-(--space-6)">Damaged Products</h2>
